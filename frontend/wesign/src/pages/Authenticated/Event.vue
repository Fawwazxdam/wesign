...
import { Plus, Search, Filter, Calendar, Users, MoreVertical, Edit, Trash2, Eye, Loader2 } from "lucide-vue-next";
...
const createEvent = async () => {
  ...
  try {
    ...
    const newEvent = {
      ...
      statusColor: getStatusColor(formData.value.status)
    };
...
const updateEvent = async () => {
  ...
  try {
    ...
    const response = await eventsApi.update(formData.value.id, payload);
    // Response interceptor already returns response.data
    const updatedEvent = response.data || response;
    const { date, time } = parseDateTime(updatedEvent.date);
    ...
    if (index !== -1) {
      events.value[index] = {
        ...
        date,
        time,
        statusColor: getStatusColor(updatedEvent.status)
      };
    }
...
const getMockEvents = () => [
  {
    id: 1,
    name: "Rapat Direksi PT ABC",
    date: "2026-02-20",
    time: "09:00",
    location: "Ruang Rapat Lt. 5",
    description: "Rapat koordinasi direksi",
    guests: 15,
    status: "Akan Datang",
  ...
  {
    id: 2,
    name: "Workshop E-Signature",
    date: "2026-02-22",
    time: "13:00",
    location: "Auditorium",
    description: "Workshop penggunaan tanda tangan elektronik",
    guests: 100,
    status: "Akan Datang",
    statusColor: "bg-blue-100 text-blue-700",
  },
  {
    id: 3,
    name: "Pelatihan Digital Signature",
    date: "2026-02-15",
    time: "10:00",
    location: "Online (Zoom)",
    description: "Pelatihan tanda tangan digital",
    guests: 50,
    status: "Selesai",
  ...
  {
    id: 4,
    name: "Seminar Legal Tech",
    date: "2026-02-10",
    time: "14:00",
    location: "Hotel Grand",
    description: "Seminar teknologi hukum",
    guests: 200,
    status: "Selesai",
    statusColor: "bg-green-100 text-green-700",
  },
  {
    id: 5,
    name: "Meeting Investor",
    date: "2026-02-25",
    time: "11:00",
    location: "Ruang VIP",
    description: "Meeting dengan investor",
    guests: 8,
    status: "Dijadwalkan",
  ...
  },
  {
    ...
    description: "Peluncuran produk baru",
    guests: 300,
    status: "Dijadwalkan",
    statusColor: "bg-yellow-100 text-yellow-700",
  },
...
<template>
  <div class="space-y-6">
      ...
      <div class="px-6 py-4 border-t border-gray-100 bg-gray-50">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div class="flex items-center gap-2">
            <button
              class="px-3 py-1 text-sm border border-gray-200 rounded-lg hover:bg-white transition disabled:opacity-50"
              disabled
            >
              Sebelumnya
            </button>
            <button
              class="px-3 py-1 text-sm bg-teal-600 text-white rounded-lg hover:bg-teal-700 transition"
            >
              1
            </button>
            <button
              class="px-3 py-1 text-sm border border-gray-200 rounded-lg hover:bg-white transition"
            >
              2
            </button>
            <button
    ...
    <!-- Create Modal -->
    <Modal :show="showCreateModal" title="Tambah Acara Baru" size="md" @close="showCreateModal = false">
    ...
    <!-- Edit Modal -->
    <Modal :show="showEditModal" title="Edit Acara" size="md" @close="showEditModal = false">
    ...
    <!-- View Modal -->
    <Modal :show="showViewModal" title="Detail Acara" size="md" @close="showViewModal = false">
      <div class="space-y-4">
        <div class="flex items-center justify-between gap-3">
          <div class="flex items-center gap-3">
            ...
            <div>
              <p class="text-sm text-gray-500">Nama Acara</p>
              <p class="font-semibold text-gray-900">{{ formData.title }}</p>
            </div>
          </div>
          <div>
            <span :class="[formData.statusColor || getStatusColor(formData.status), 'px-3 py-1 rounded-full text-xs font-medium inline-block mt-1']">
              {{ formData.status }}
            </span>
          </div>
        </div>
        <div class="grid grid-cols-2 gap-4">
          <div>
            ...
            <p class="font-medium text-gray-900">{{ formData.date || '-' }}</p>
          </div>
          <div>
            <p class="text-sm text-gray-500">Waktu</p>
            <p class="font-medium text-gray-900">{{ formData.time ? formData.time + ' WIB' : '-' }}</p>
          </div>
        </div>
        <div>
          <p class="text-sm text-gray-500">Lokasi</p>
          <p class="font-medium text-gray-900">{{ formData.location || '-' }}</p>
        </div>
        <div>
          <p class="text-sm text-gray-500">Deskripsi</p>
          <p class="font-medium text-gray-900">{{ formData.description || 'Tidak ada deskripsi' }}</p>
        </div>
      </div>
      <template #footer>
    ...
    <Modal :show="showDeleteModal" title="Hapus Acara" size="sm" @close="showDeleteModal = false">
      <div class="py-4">
